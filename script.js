...
/* =========================================================
   Detailed Center â€” Interactions
   ========================================================= */
...
(function () {
  ...
  if (typeof Lenis !== 'undefined') {
    // Inicializamos Lenis SIEMPRE, ignorando prefers-reduced-motion para garantizar el efecto
    lenis = new Lenis({
      duration: 1.5,
    ...
    console.log('[Lenis] smooth scroll forzado activo');
  } else {
  ...
  const smoothScrollTo = (targetY) => {
    if (lenis) {
      lenis.scrollTo(targetY, { offset: -60 });
    } else {
      window.scrollTo({ top: targetY - 60, behavior: 'smooth' });
    }
  };
  // Anchor links
  ...
  if (heroTitle) {
    ...
    words.forEach((w, i) => { w.style.transitionDelay = `${120 + i * 90}ms`; });
    requestAnimationFrame(() => {
      requestAnimationFrame(() => heroTitle.classList.add('in'));
    });
  }

  /* ---------- Reviews marquee â€” JS-driven for iOS Safari reliability ---------- */
  (function setupMarquee() {
    const marquee = document.getElementById('reviewsMarquee');
    if (!marquee) return;
    // Clone enough times to ensure overflow on wide screens
    const original = [...marquee.children];
    for (let i = 0; i < 2; i++) {
      original.forEach((it) => marquee.appendChild(it.cloneNode(true)));
    }
    // Disable CSS animation, drive with rAF
    ...
    let paused = false;
    let halfWidth = 0;
    const recalc = () => { halfWidth = marquee.scrollWidth / 3; };
    recalc();
  ...
  /* ---------- Services hover-reveal sticky stage ---------- */
  (function setupServices() {
    ...
    items.forEach(i => {
      ...
      if (i.dataset.imgSecondary) sources.add(i.dataset.imgSecondary);
    });
    ...
    const updateVisibility = () => {
      ...
      if (activeItem) {
        ...
        const sSrc = activeItem.dataset.imgSecondary;
        if (sSrc && !showingPrimary) {
          if (imgMap[sSrc]) imgMap[sSrc].classList.add('is-visible');
        } else {
    ...
    const startCrossfade = (item) => {
      if (crossfadeInterval) clearInterval(crossfadeInterval);
      showingPrimary = true;
      if (item.dataset.imgSecondary) {
  ...
  (function setupCompare() {
    ...
    const onUp = () => { dragging = false; compare.classList.remove('is-dragging'); };
    ...
    compare.addEventListener('keydown', (e) => {
      const cur = parseInt(compare.getAttribute('aria-valuenow') || '50', 10);
      let next = cur;
      if (e.key === 'ArrowLeft') next = Math.max(0, cur - 4);
      if (e.key === 'ArrowRight') next = Math.min(100, cur + 4);
      if (next !== cur) {
        const rect = compare.getBoundingClientRect();
        setPos(rect.left + (rect.width * next) / 100);
        e.preventDefault();
      }
    });
